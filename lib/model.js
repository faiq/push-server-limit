...
Ratelimit.prototype.add = function (userID, route) { 
  var _this = this
    ...
    var script = '\
      local current \
      current = redis.call("hincrby", KEYS[1], KEYS[2], 1) \
      if tonumber(current) == 1 then \
        redis.call("hset", KEYS[1], "firstRequestTime", KEYS[3]) \
      end \
    '
    multi.eval(script, 3, lookup, "count", currentTime)   
    .expire(lookup, _this.expire) 
    .exec(function (err, replies) { 
      if (err) throw Error("something went horribly wrong with the redis transaction") 
    })
